...
# RPM & DEB packages
If rpmbuild is installed an rpm package will be build for rpm based distributions.
On debian based distribution a deb package will also be build if both rpmbuild and alien are installed.
Instead of using the custom installer script you can use these OS packages to install, uninstall or update restit.
